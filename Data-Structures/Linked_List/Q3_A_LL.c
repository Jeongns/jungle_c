    ...
    while (c != 0) {
        ...
        switch (c) {
            ...
            case 2:
                ...
                printf(
                    "The resulting linked list after moving odd integers to the back of the linked "
                    "list is: ");
                printList(&ll);
                removeAllItems(&ll);
                break;
...
void moveOddItemsToBack(LinkedList *ll) {
	...
	if(oddCurrentNode != NULL) oddCurrentNode->next = NULL;

	ll->head = oddNodeHeaader;

	if (evenNodeHeader != NULL) {
		ll->head = evenNodeHeader;
		evenCurrentNode->next = oddNodeHeaader;
	}
}
